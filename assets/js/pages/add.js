...
const getCapitulos = async () => {
  const response = await axios.get('https://iee-uso.herokuapp.com/api/capitulos');
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (idVoluntario) {
      try {
        response = await axios.put(`https://iee-uso.herokuapp.com/api/voluntarios/${idVoluntario}`, formData, {
          headers: {
            'x-token': token,
          },
        });
      ...
      try {
        response = await axios.post('https://iee-uso.herokuapp.com/api/voluntarios', formData, {
          headers: {
            'x-token': token,
          },
        });
    ...
    if (data.status) {
      ...
      Swal.fire({
        icon: 'success',
        title: !idVoluntario ? 'Se registro correctamente' : 'Se actualizo correctamente',
        showConfirmButton: false,
        timer: 1500,
      });
    }
    ...
    console.log(error);
    let mensajeError = error?.response?.data?.mensaje || error?.response?.data?.msj || null;
    if (!mensajeError) {
...
const validarEditar = async () => {
  ...
  if (id) {
    idVoluntario = id;
    const resp = await axios.get(`https://iee-uso.herokuapp.com/api/voluntarios/${id}`);
